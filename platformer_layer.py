...
def check_space(main_object, objects):
    ...
    порядок в списке: вверх, право, низ, лево
     """
    rect_up = pygame.Rect(main_object.x - main_object.w / 2, main_object.y - main_object.h / 2 - 1, main_object.w, 2)
    rect_right = pygame.Rect(main_object.x + main_object.w / 2 + 1, main_object.y - main_object.h / 2, 1, main_object.h/2)
    rect_down = pygame.Rect(main_object.x - main_object.w / 2, main_object.y + main_object.h / 2 + 1, main_object.w, 2)
    rect_left = pygame.Rect(main_object.x - main_object.w / 2 - 1, main_object.y - main_object.h / 2, 1, main_object.h/2)
    free_spaces = [False, False, False, False]

    for object in objects:
        rect_object = pygame.Rect(object.x - object.w / 2, object.y - object.h / 2 + 1, object.w, object.h)
